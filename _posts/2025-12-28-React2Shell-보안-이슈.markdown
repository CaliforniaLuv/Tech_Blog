...
당시 사내 개발팀 슬랙(Slack)에서도 해당 이슈가 긴급히 공유되었고, 저희 팀은 서비스 안전성을 위해 즉각적인 버전 업데이트를 완료했다.
...
위험 등급 스코어가 무려 ___10.0___ 이 나왔는데 이정도 점수는 나오기도 힘들다 들었다. (= 박평식 평론가 별점 5개 급)

해결책은 버전 업데이트로 명확했으나, 서비스 영향도를 고려해야 했다.

홈페이지부터 웹앱 기반 서비스까지 React로 긴밀하게 연결되어 있어, 사소한 업데이트가 자칫 예상치 못한 장애로 이어질 수 있었기 때문인데
이를 방지하기 위해 개발팀원들과 함께 3시간에 걸쳐 고강도 집중 QA를 실시하며 꼼꼼히 체크했다.
...
이러한 위기감을 바탕으로, 실제 어떤 경로로 공격이 이루어질 수 있는지 확인하기 위해 로컬 환경에서 재현 테스트를 해보기로 하였다.
