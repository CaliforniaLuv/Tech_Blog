...
당시 사내 개발팀 Slack 채널에서도 해당 이슈가 긴급히 공유되었고, 우리 팀은 서비스 안전성을 위해 즉각적인 버전 업데이트를 완료했다.
...
이 숫자 키들은 배열 인덱스처럼 보이지만,
실제로는 React Flight 내부에서 객체/참조를 연결하는 테이블 역할을 한다.

즉 $1, $2, $3 같은 값들은 “나중에 이 인덱스를 다시 참조해” 라는 의미이며 ___객체 그래프를 재조립___ 하는 설계 과정이 된다.

가장 핵심인 아래 코드가 React2Shell의 원인이 되는 트릭이며,
...
![캡쳐_이미지6](https://californialuv.github.io/Tech_Blog/images/2025-12-28/6.png)

obj.constructor.constructor 는 javascript 생태계에서 Function 생성자로 취급되며 즉 아래 처럼 해석할 수 있다.
